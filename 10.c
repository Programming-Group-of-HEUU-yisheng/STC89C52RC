/*
	Author: Fancyguy  wongsyrone
	Big Thanks to Fancyguy for contributing to judgement part.
		He widen my horizon.
	And, I will attach my previous version of this for comparison.
	...
	IMPORTANT: judgement statements part by Fancyguy
...
{
	uchar i,j;
	while (num <= 20)  				//to keep lighting for the fisrt 1s
	{
		for (i=0; i<6; i++)
		{
			dula=1;
	...
	while(num <= 40 && num > 20)			 // to shut down for the first 1s
	{
		dula=1;
		P0=0x00;
		dula=0;
	...
	while (num <= 60)    	 		// to keep lighting for the second 1s
	{
		for (j=0; j<6; j++)
		{
			dula=1;
	...
	while(num <= 80 && num > 60) 		//to shut down for the second 1s
	{
		dula=1;
		P0=0x00;
		dula=0;
